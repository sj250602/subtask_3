...
void resolution_change(int x, int y, vector<pair<int,pair<double,double>>> &st, VideoCapture cap, Mat &emptyframe){
	Mat prev_image = emptyframe;

	int frame_num = 0;
	int fps_3 = -1;
	double frames= cap.get(CAP_PROP_FRAME_COUNT);
	Size toreso = Size(x,y);
	
	while (fps_3<frames)
	{
		fps_3++;
		Mat curr_frame,gray_prev,gray_emp,crop_emp,crop_curr,crop_prev,gray_curr,small_curr_image, small_prev_image, small_empty_image;
		...
		if(fps_3%3==1){
			...
			resize(crop_emp, small_empty_image, toreso,0 , 0, INTER_LINEAR);
  			
  			cvtColor (prev_image , gray_prev, COLOR_BGR2GRAY);
  			crop_prev = crop_frame(gray_prev);
			resize(crop_prev, small_prev_image, toreso,0 , 0, INTER_LINEAR);
  			
  			cvtColor (curr_frame, gray_curr, COLOR_BGR2GRAY);
			crop_curr = crop_frame(gray_curr);
			resize(crop_curr, small_curr_image, toreso,0 , 0, INTER_LINEAR);
		
			getcontoursarea(small_curr_image,small_empty_image,small_prev_image);
...
int main(int argc, char* argv[])
{
	...
	resolution_change(x_coor, y_coor, myvector, cap, emptyframe);
	long final = gettime();
	long total_runtime = final-initial;
	cout<< "Runtime is " << to_string(total_runtime) <<endl;
 	ofstream store_coor;
	string m = "resolution" + to_string(x_coor) + "to" + to_string(y_coor) + ".txt";
	store_coor.open(m);
	for (auto k: myvector){
